    ...
    public class UsuarioLogica
    {
        ...
        public Usuario Obtener(string _correo, string _contrasena)
        {
            ...
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN1))
            {
                try
                {
                    SqlCommand cmd = new SqlCommand("sp_obterUsuario", oConexion);
                    cmd.Parameters.AddWithValue("Email", _correo);
                    cmd.Parameters.AddWithValue("Senha", _contrasena);
                    ...
                    using (SqlDataReader dr = cmd.ExecuteReader()) {
                        while (dr.Read()) {
                            objeto = new Usuario()
                            {
                                ...
                                Nombres = dr["Nomes"].ToString(),
                                Apellidos = dr["Apelido"].ToString(),
                                Correo = dr["Email"].ToString(),
                                Contrasena = dr["Senha"].ToString(),
        ...
        public int Registrar(Usuario oUsuario)
        {
            ...
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN1))
            {
                try
                {
                    ...
                    cmd.Parameters.AddWithValue("Nomes", oUsuario.Nombres);
                    cmd.Parameters.AddWithValue("Apelido", oUsuario.Apellidos);
                    cmd.Parameters.AddWithValue("Email", oUsuario.Correo);
                    cmd.Parameters.AddWithValue("Senha", oUsuario.Contrasena);
